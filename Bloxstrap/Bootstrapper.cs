            ...
            try
            {
                ...
                try
                {
                    ...
                    if (App.Settings.Prop.EnableAntiAFK)
                        Utility.AntiAFKManager.Start(_robloxProcess);
            ...
            var blockedFlags = new HashSet<string>
{
    ...
    "FFlagEnableNapIxpLayerExposure_IXPValue"
        ...
        private void LaunchExternalTools()
        {
            const string LOG_IDENT = "Bootstrapper::LaunchExternalTools";
            
            try
            {
                if (App.Settings.Prop?.ExternalTools == null || App.Settings.Prop.ExternalTools.Count == 0)
                    return;

                App.Logger.WriteLine(LOG_IDENT, $"Launching {App.Settings.Prop.ExternalTools.Count} external tool(s)");
                ...
                foreach (var tool in App.Settings.Prop.ExternalTools)
                {
                    try
                    {
                        ...
                        {
                            App.Logger.WriteLine(LOG_IDENT, $"Skipping '{tool.Name}' - file not found: {tool.Path}");
                            continue;
                        }
                        ...
                        App.Logger.WriteLine(LOG_IDENT, $"Launched '{tool.Name}': {tool.Path}");
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteLine(LOG_IDENT, $"Failed to launch '{tool.Name}': {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine(LOG_IDENT, $"Error launching external tools: {ex.Message}");
            }
        }
        #endregion
    }
